...
// ── KPE: ไม่ใช้ service worker ───────────────────────────────────────────────
// แอปเสิร์ฟจาก server ใน LAN ของตัวเอง — SW (PWA cache) ไม่ให้ประโยชน์ (server ดับ = ใช้ไม่ได้อยู่ดี)
// แต่ทำให้ "แอปเก่าค้าง" หลัง deploy (ต้อง hard refresh 2 รอบ) → ตัดทิ้ง แล้วคุม cache ที่ serve.js แทน
// (no-cache + ETag — LAN revalidate เร็วมาก ดู serve.js/deploy.js)
if ('serviceWorker' in navigator) {
  // ถอน SW เก่าที่เครื่อง client เคยติดไว้จาก build ก่อน ๆ — เปิดครั้งถัดไปก็หายค้างถาวร
  navigator.serviceWorker.getRegistrations()
    .then((rs) => rs.forEach((r) => r.unregister()))
    .catch(() => {});
}

_flutter.loader.load();   // ไม่ใส่ serviceWorkerSettings = ไม่ register SW ใหม่
